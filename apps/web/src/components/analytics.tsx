...
export function event(event: string, action: string, data: Record<string, any>) {
  if (ANALYTICS.driver === 'ga4' && ANALYTICS.id && window.gtag) {
    ...
    console.warn('[GA4] Failed to send event. Check if ANALYTICS.id and window.gtag are available.');
  }
}

export function page(event: string, action?: string, data?: Record<string, any>) {
  if (ANALYTICS.driver === 'ga4' && ANALYTICS.id) {
    console.log(`[GA4] Page event tracked: ${event}, Action: ${action || 'default'}, Data:`, data || {});
  }
}
  ...
  if (ANALYTICS.driver === 'ga4' && ANALYTICS.id) {
    return (
      <>
        ...
        {!ANALYTICS.id.startsWith('GT-') && <GoogleAnalytics gaId={googleAnalyticId} />}
      </>
    );
  }
