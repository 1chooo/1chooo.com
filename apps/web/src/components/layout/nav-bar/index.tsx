      ...
      <ul className="flex flex-wrap justify-center items-center px-2.5 sm:gap-5 lg:gap-[30px] lg:px-5">
        {navItems.map((item) => (
          <li key={item.path}>
            <ProgressBarLink
              href={item.path}
              className={`navbar-link text-light-gray md:text-[15px] sm:text-[14px] text-[11px] hover:text-light-gray-70 ${isActive(item.path) ? "active text-orange-yellow-crayola font-bold" : ""}`}
            >
              {item.label}
            </ProgressBarLink>
          </li>
